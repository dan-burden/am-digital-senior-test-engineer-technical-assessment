...
test("basic search box goes to results page", async ({ page }) => {
	...
	await expect(page.getByTestId('search-criteria-search-terms-group')).toHaveText(/Brown/)
	// assert search result
	await expect(page.getByText('1 April 1875. Browning, Robert to Pollock, Lady')).toBeVisible()
});
...
test("multi word search has multiple search terms", async ({ page }) => {
	...
	await expect(page.getByTestId('search-criteria-search-terms-group')).toHaveText(/Browning/)
	await expect(page.getByTestId('search-criteria-search-terms-group')).toHaveText(/Pollock/)
	// assert results
	await expect(page.getByText('1 April 1875. Browning, Robert to Pollock, Lady')).toBeVisible()
});